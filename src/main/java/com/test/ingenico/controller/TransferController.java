...
import com.test.ingenico.model.Response;
...
public class TransferController {
	...
	public ResponseEntity createAccount(@RequestBody AccRequest req)throws TransferSvcException
	{
		...
		if(req==null)
			throw new TransferSvcException(TransferConstants.REQUESTNULL,"400");
		...
		try {
			...
			//create Account
			 accountNumber=transferService.createAccount(req);
			...
			resp.setCode(TransferConstants.HTTP_OK);
			resp.setStatus(accountNumber);
			return new ResponseEntity<Response>(resp,HttpStatus.OK);
		} catch (TransferSvcException e) {
			resp.setStatus(e.getErrorMsg());
			resp.setCode(TransferConstants.HTTP_BADREQUEST);
		...
		}catch(TransferSvcException e)
		{
			...
			if(e instanceof ObjectOptimisticLockingFailureException)
			{
				...
				{	
					...
					resp.setStatus(status);
					return new ResponseEntity<Response>(resp,HttpStatus.OK);
				}else return new ResponseEntity<String>(TransferConstants.RETRYLIMIT,HttpStatus.OK);
			}
			else
